...
echo "-----------------------------------------------------------------------------------------------"
echo "Hihi starts releasing (Azure Container Registry)"
echo "-----------------------------------------------------------------------------------------------"
echo "\n"
date
pwd

# Load ACR_NAME (and any other overrides) from .env next to this script.
...
if [ -z "$ACR_NAME" ]; then
    ...
    echo "       Copy $SCRIPT_DIR/.env.example to $SCRIPT_DIR/.env and fill in ACR_NAME,"
    echo "       or export ACR_NAME in your shell before running this script."
    exit 1
fi
